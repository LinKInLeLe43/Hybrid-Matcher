...
class MegaDepthDataset(Dataset):
    ...
    def get_name(self, modality: str, idx: int):
        name = osp.join(self.modality_to_root[modality], self.scene_info['image_paths'][idx])
        if modality == 'event':
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            modality = self.rng.choice(self.modality_list)
            if self.rng.randint(0, 1) == 0:
                img_name0, img_name1 = self.get_name('visible', idx0), self.get_name(modality, idx1)
            else:
                img_name0, img_name1 = self.get_name(modality, idx0), self.get_name('visible', idx1)
        else:
            assert len(self.modality_list) == 1
            img_name0, img_name1 = self.get_name('visible', idx0), self.get_name(self.modality_list[0], idx1)
