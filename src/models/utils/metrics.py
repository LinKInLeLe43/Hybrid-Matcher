...
def _compute_end_point_errors(
    ...
    fine_points0: torch.Tensor,
    fine_points1: torch.Tensor,
    depth0: torch.Tensor,
    depth1: torch.Tensor,
    K0: torch.Tensor,
    K1: torch.Tensor,
    ...
    for b in range(n):
        ...
        gt_fine_points1, mask = _warp_point(
            fine_points0[b_mask][None], depth0[[b]], depth1[[b]], K0[[b]],
            K1[[b]], T0_to_1[[b]], return_mask=True,
        ...
        if coarse_points0 is not None:
            gt_coarse_points1, mask = _warp_point(
                coarse_points0[b_mask][None], depth0[[b]], depth1[[b]], K0[[b]],
                K1[[b]], T0_to_1[[b]], return_mask=True,
                consistent_depth_ratio=consistent_depth_ratio)
            gt_coarse_points1, mask = gt_coarse_points1[0], mask[0]

        end_point_errors = (
            (fine_points1[b_mask][mask] - gt_fine_points1[mask]) /
            b_scale1).norm(dim=1)
        ...
        extra_gt_j_idxes = (coarse_w1 // 2 * extra_gt_coarse_grid1[:, :, 1] +
                            extra_gt_coarse_grid1[:, :, 0])
        extra_coarse_topk_results = (extra_j_topk_idxes[b_mask][mask] ==
                                     extra_gt_j_idxes).float()
        ...
        if len(extra_coarse_topk_results) != 0:
            extra_coarse_topk_precision = extra_coarse_topk_results.sum(dim=1).mean().item()
        ...
        inlier_extra_coarse_topk_precisions.append(inlier_extra_coarse_topk_precision)
    ...
    return (end_point_errors_per_batch, inlier_end_point_errors_per_batch,
            ...
            coarse_recalls, extra_coarse_topk_recalls)
    ...
    if advanced:
        ...
        (error["end_point_errors_per_batch"],
         ...
         error["extra_coarse_topk_recalls"]
            ...
            result["coarse_recall"], result["extra_coarse_topk_recall"],
            inliers_per_batch, coarse_points0=result.get("coarse_points0"))
    return error
...
def compute_metric(
    ...
    if advanced:
        ...
        metric["extra_coarse_topk_precision"] = (
        ...
        metric["coarse_recall"] = (
            error["coarse_recalls"][idxes].mean())
