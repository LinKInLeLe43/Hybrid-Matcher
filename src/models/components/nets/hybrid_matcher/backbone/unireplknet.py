...
UniRepLKNet_A_F_P_depths = (2, 2, 6)
...
class UniRepLKNet(nn.Module):
    ...
    def __init__(self,
        ...
        if kernel_sizes is None:
            kernel_sizes = default_UniRepLKNet_A_F_P_kernel_sizes
        print(kernel_sizes)
        for i in range(3):
        ...
        self.downsample_layers.append(nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(dims[0] // 2, dims[0], kernel_size=3, stride=1, padding=1),
...
if __name__ == '__main__':
    model = UniRepLKNet(depths=UniRepLKNet_A_F_P_depths, layer_depths=(128, 196, 256))
    x = torch.rand((2, 1, 832, 832))
    output = model(x)
    a = 1