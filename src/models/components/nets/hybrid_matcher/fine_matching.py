...
class FineMatching(nn.Module):  # TODO: change name to second stage
    def __init__(
        ...
        if type == "one_stage":
            ...
            grid1 = K.create_meshgrid(cls_w + 2 * cls_w, cls_w + 2 * cls_w, normalized_coordinates=False)
            biases1 = (grid1 - cls_w - cls_w // 2 + 0.5).reshape(-1, 2)
