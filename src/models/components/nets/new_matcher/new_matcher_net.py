    ...
    def forward(
        ...
        if batch["image0"].shape == batch["image1"].shape:
            ...
            with torch.no_grad():
                ...
                h, w = image.shape[2:]
                x = F.interpolate(
                    norm_image, size=(h // 16 * 14, w // 16 * 14), mode="bicubic"
                )
        ...
        vit0, vit1 = vit_features[-1][0].transpose(1,2).unflatten(2, (h // 16, w // 16)).chunk(2)

        x0_16x, x1_16x = self.coarse_module(
            x0_16x, x1_16x, vit0, vit1, rope=self.rope, mask0=mask0_16x, mask1=mask1_16x)
