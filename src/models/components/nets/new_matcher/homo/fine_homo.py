...
class FineHomo(nn.Module):
    ...
    def initialize_coords(self):
        device = (
            torch.device(f"cuda:{rank_zero_only.rank}")
    ...
    def forward(self, feat_f0_unfold, feat_f1_unfold, iters_lev0, biases=None):
        """
        Input:
        ...
        sim_matrix = torch.einsum(
            "mlc,mrc->mlr", feat_f0_unfold, feat_f1_unfold
        )
        ...
        if self.padding:
            ...
            coords0, coords1 = (
                self.coords0[M].to(fmap1.dtype),
                self.coords1[M].to(fmap1.dtype),
            )
        ...
        if biases is not None:
            points, r_points = ((self.W - 1) * biases).chunk(2, dim=1)
            points = torch.cat(
