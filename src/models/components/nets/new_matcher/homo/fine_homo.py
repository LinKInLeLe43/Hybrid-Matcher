...
class FineHomo(nn.Module):
    ...
    def get_flow_now_k(self, four_point, k = 1):
        ...
        if N not in self.points:
            self.points[N] = torch.cat((self.coords0[N].view(N, 2, -1), torch.ones((N, w * h)).unsqueeze(1).to(four_point.device)), dim=1)
        ...
        flow = torch.cat((points_new[:, 0, :].reshape(N, h, w).unsqueeze(1),
                          points_new[:, 1, :].reshape(N, h, w).unsqueeze(1)), dim=1)
    ...
    def forward(self, feat_f0_unfold, feat_f1_unfold, iters_lev0, init=None):
        ...
        M_, W_W, dim = feat_f0_unfold.shape
        ...
        sim_matrix = torch.einsum('mlc,mrc->mlr', feat_f0_unfold, feat_f1_unfold)
        corr = sim_matrix.view(-1, self.W, self.W, 1, self.W, self.W)

        fmap1 = feat_f0_unfold.view(M, self.W, self.W, dim).permute(0,3,1,2) # [M, W,W,dim]]
        fmap2 = feat_f1_unfold.view(M, self.W, self.W, dim).permute(0,3,1,2) # # [320, 16, 16]]
        ...
        for itr in range(iters_lev0):
            ...
            if iters_lev0 > itr + 1:
                coords1, H = self.get_flow_now_k(four_point_disp, k=1) # self.sz[-1]//sz[-1])
            else:
    ...
    def get_fine_match_dense(self, dense_points, warp_dense_points, data):        
        ...
        b_ids, i_ids, j_ids = data['b_ids'],data['i_ids'],data['j_ids']
        mkpts0_c = torch.stack([i_ids % data['hw0_c'][1], i_ids // data['hw0_c'][1]], dim=1) * 4
        mkpts1_c = torch.stack([j_ids % data['hw1_c'][1], j_ids // data['hw1_c'][1]],dim=1) * 4
        mkpts0_f = mkpts0_c.reshape(-1, 1, 1, 2) + (dense_points-W // 2)[:len(data['mconf'])]
        mkpts0_f = mkpts0_f.view(-1,2)
        mkpts1_f = mkpts1_c.reshape(-1, 1, 1, 2) + (warp_dense_points-W // 2)[:len(data['mconf'])]
        mkpts1_f = mkpts1_f.reshape(-1,2)
        h_f, w_f = data['hw0_f']
        dense_warp = torch.ones(h_f, w_f, 2).to(mkpts0_c.device)*(max(h_f, w_f)+1)
        ...
        # })
        data.update({
            "mkpts0_f": mkpts0_c * scale * data['scale0'][data['b_ids']], #[self.dense_num**2//2:][::self.dense_num**2],
            "mkpts1_f": mkpts1_f[0::16] * scale * data['scale1'][data['b_ids']] #[self.dense_num**2//2:][::self.dense_num**2]
        })
    ...
    def get_fine_match(self, coords_normed, data):        
        ...
        if self.training and 'expec_f_inverse' in data:
            ...
            scale0 = scale * data['scale0'][data['b_ids']] if 'scale0' in data else scale
            mkpts0_f_ = data['mkpts0_c'] + (coords_normed_inverse * (W // 2) * scale0)[:len(data['mconf'])]
    ...
    def get_coor_std(self,corr_fn, M, warp_points,M_, data,offset):
        corr_map = corr_fn.corr_map.view(M,self.W,self.W,self.W,self.W)[:,self.W//2,self.W//2,:,:]
        sim_matrix_exp = F.softmax(corr_map.view(M, -1), dim=1)
        positive_indices = 2 * warp_points.clone() / (self.W - 1) - 1
        sim_matrix_exp = F.grid_sample(sim_matrix_exp.view(M, 1, self.W, self.W), 
                                   positive_indices.unsqueeze(1).unsqueeze(1), 
                                   align_corners=True)
        data.update({'conf_f': sim_matrix_exp[:M_].view(M_,-1)})
  
        data.update({'expec_f': torch.cat([offset, sim_matrix_exp[:M_].view(M_,-1)], -1)}) 
