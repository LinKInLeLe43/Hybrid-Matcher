...
class GlobalCoC(nn.Module):
    ...
    def forward(
        ...
        for merge_block, global_block, self_block, cross_block in zip(
            ...
            x0 = self_block(
                x0, x0, rope=rope, x_mask=y0_mask, y_mask=y0_mask)
            x1 = self_block(
                x1, x1, rope=rope, x_mask=y1_mask, y_mask=y1_mask)
            x0 = cross_block(x0, x1, x_mask=y0_mask, y_mask=y1_mask)
            x1 = cross_block(x1, x0, x_mask=y1_mask, y_mask=y0_mask)
        return x0, x1
