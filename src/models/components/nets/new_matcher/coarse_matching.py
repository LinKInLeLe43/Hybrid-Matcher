...
class CoarseMatching(nn.Module):
    ...
    def _create_coarse_matching(
        self,
        score: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor],
        size0: Tuple[int, int],
        size1: Tuple[int, int],
        mask0: Optional[torch.Tensor],
        mask1: Optional[torch.Tensor],
        ...
        b_idxes, i_idxes = mask.nonzero().unbind(dim=-1)
        j_idxes = idxes0_to_1[b_idxes, i_idxes]
        train_idxes = matching_idxes = torch.stack([b_idxes, i_idxes, j_idxes], dim=-1)
    ...
    def forward(
        ...
        n, c, h0, w0 = x0.shape
        _, _, h1, w1 = x1.shape
        fh0, fw0 = h0 // self.scale, w0 // self.scale
        fh1, fw1 = h1 // self.scale, w1 // self.scale
        ...
        x0 = _x0.reshape(n, fh0, fw0, self.scale, self.scale, c).permute(0, 5, 1, 3, 2, 4).reshape(n, c, h0, w0)
        x1 = _x1.reshape(n, fh1, fw1, self.scale, self.scale, c).permute(0, 5, 1, 3, 2, 4).reshape(n, c, h1, w1)
        # x0 = rearrange(
        ...
        similarity0_to_1 = similarity0_to_1.reshape(n, fh0, fw0, self.scale, self.scale, -1).permute(0, 1, 3, 2, 4, 5).reshape(n, h0 * w0, -1)
        similarity1_to_0 = similarity1_to_0.reshape(n, -1, fh1, fw1, self.scale, self.scale).permute(0, 1, 2, 4, 3, 5).reshape(n, -1, h1 * w1)
